...
#### S3.7 — Personal book vault, Android client half (5 pt) — **client proven, not yet reachable**
...
- [x] Client-side AES-GCM encryption; the key derives from the user's own secret and is never transmitted — the server stores ciphertext it cannot read
- [x] **Per-user namespacing in the primary key**, structurally rather than by convention (§8.2(1)) — including any local mirror of translation-cache rows
- [ ] **Opt-in per book is NOT built.** The repository honours an opt-out (proven: an opted-out book reaches the gateway zero times) but no UI control sets it, so the vault is off and cannot be turned on
- [x] Upload / download / restore, such that a book restored on a second device re-bills **nothing**
- [x] The account screen tells the truth about the vault without overstating it — the agent **removed** "from that book's page" from the copy because it promised a control that does not exist
- [x] Room v7 → v8, additive, schema asserted via `PRAGMA table_info` — not just row counts
...
- **Verify run 2026-07-27 on merged `main`:** **1237 JVM tests** (666 debug incl. 52 skipped + 571 release), **0 failures**, from a 1161 baseline the agent measured itself. **€0.** Supervisor-verified with 0 skipped across all four vault suites: `VaultRoundTripTest` 4/4 — including *a book restored from the vault onto a second device translates for zero API calls*, asserting `callCount == 0` with **no scripted replies and no exhaustion handler**, so any call fails loudly rather than returning something plausible; `VaultIsolationTest` 9/9; `VaultCryptoTest` 10/10; `Migration7To8Test` 10/10.
- **Per-user keying is structurally unavoidable, in three independent layers** — because §8.3(1) is the legal posture, not a preference. **Type:** `UserId` is a value class, so it cannot be passed where a `bookHash` is expected. **Key:** `userId` leads all three PKs, asserted via `PRAGMA table_info`. **API:** `VaultRepository` binds its owner at construction; no method takes a `userId` and `VaultGateway` has no overload without one — the forbidden call is *unspeakable*. Mutation-proven both ways: dropping `userId` from the PK fails the key test, and keying hosted rows without the real owner (simulating the cross-user dedup §8.3(1) forbids) fails **four** tests including the headline.
- **The test names cite their contract clauses** (`sync_api 8_2_1`, `8_3_1`, `8_3_2`, `8_3_3`…), so each constraint is traceable from the suite back to the authority it encodes.
- **RESIDUAL, and it is the S2.6 shape — this story is not done.** There is no production `VaultGateway` and no per-book opt-in control, so `VaultRepository` is **not wired into `AppContainer`** and **no end-to-end path from user gesture to stored result has been walked**. The vault is proven behind its seam and unreachable in a real build. The server half is `berilo-cloud`'s per CLAUDE.md §5. Building a speculative HTTP client against an unimplemented contract would have been worse than not building one — but §9's rule is explicit that a feature is not done until that path is walked, and it has not been.
- **Reported, not fixed:** the local `translations`/`glossaries` tables are not user-scoped — correct per §8.3(1) for one machine, but two accounts on one physical device could cross-pollinate through the shared local cache when uploading. The one place the per-user invariant rests on circumstance rather than structure.
